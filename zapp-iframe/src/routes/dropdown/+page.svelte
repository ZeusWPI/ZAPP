<script lang="ts">
	...
	const apps = [
		...
		{ img: 'zess.svg', url: 'https://zess.zeus.gent', tooltip: 'ZESS: Kelder logs', visible: false },
		{
			img: 'Mattermost_icon_denim.svg',
			url: 'https://mattermost.zeus.gent',
			class: 'mattermost',
			tooltip: 'Chat', visible: false
		},
		{ img: 'haldis_black.png', url: 'https://haldis.zeus.gent', tooltip: 'Haldis: Eten bestellen', visible: false },
		...
		{ img: 'codimd.png', url: 'https://codimd.zeus.gent', tooltip: 'CodiMD: Notities', visible: false },
		{
			img: 'profile_nobg.png',
			url: 'https://zauth.zeus.gent',
			class: 'profile',
			tooltip: 'Zauth profiel', visible: false
		}
	];
										...
										<div class="icontainer absolute">
											...
											>
												<img
													...
													alt={app.img}
													onload={(target)=>{loaded[i]=true; loaded = {...loaded}}}
													class={'w95icon easeload ' + (loaded[i] ? 'visible ': '') + (app.class ?? '')}
												/>
											</button>
										</div>
									</div>
